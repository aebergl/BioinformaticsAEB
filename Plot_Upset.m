...
if ~ islogical(Data)
    if iscell(Data)
        ...
        DataTmp = zeros(length(UniqueData),nSets);
        for i = 1:length(Data)
            DataTmp(:,i) = ismember(UniqueData,Data{i});
        end
    end
...
pBool(sum(pBool, 2) == 1, :) = []; %skip set intersections with themselves.
switch lower(mode)
    case 'distinct'
        [pPos,~]=find(((pBool*(1-Data'))|((1-pBool)*Data'))==0);
    case 'interseciton'
        [pPos,~]=find((pBool*Data')==sum(pBool,2));
end
...
%% ========================================================================
% 构造figure及axes
fh=figure('Units','normalized','Position',[.3,.2,.5,.63],'Color',[1,1,1]);
axI=axes('Parent',fh);hold on;
...
text(axI,1:length(pType),pCount,string(pCount),'HorizontalAlignment','center',...
    'VerticalAlignment','bottom','FontName','Times New Roman','FontSize',8,'Color',[61,58,61]./255)
% 集合统计图 ---------------------------------------------------------------
